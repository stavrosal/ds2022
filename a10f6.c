...
int main(){
    ...
    if (Loc != -1 && Loc2 != -1){
        if (strcmp(t1, HList.List[Loc].Data.name) == 0 && strcmp(t2, HList.List[Loc2].Data.name) == 0 )
            printf("%sx%s=%d", t1, t2, HList.List[Loc].Data.value*HList.List[Loc2].Data.value);
        else if (strcmp(t1, HList.List[Loc].Data.name) != 0)
             printf("Undeclared variable %s", t1);
        else if (strcmp(t2, HList.List[Loc2].Data.name) != 0)
             printf("Undeclared variable %s", t2);
    }
